...
You can also use the syntax from RouterOS's shell (spaces between words instead of "/"). Either way, the command needs to be absolute (begin with "/"). Note also that auto completion is not supported (e.g. "/ip/fire/nat/print" will NOT be automatically translated to "/ip/firewall/nat/print"). Examples in the rest of this documentation will use the API syntax.
