...
It's important to note that unlike the MySQLi equivalent, these persistent connections come with a penalty of their own - A penalty on the web server. Because multiple requests can come in at the same time, and they're all "meshed", PEAR2_Net_RouterOS needs to marshal the different requests and responses to each Client instance, which is done measurably slower in PHP than in C. How much slower? Anywhere between a few microseconds and a millisecond per request, depending on your server's CPU and RAM. A negligible penalty for a single request, but accumulated when you have lots of simultanious requests, which is when you'd consider using persistent connections to begin with.
